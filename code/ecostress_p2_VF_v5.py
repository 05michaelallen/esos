...
if buildings_only == "yes":
    lc = rio.open(cover)
    #lcr = lc.read()[0,:,:]
    lcr = lc.read()[0,3052:3152,4452:4552]                          ###### DELETE
...
for f in range(47, 48):                                         ##### DELETE 
    ...
    if va_quad == 1:
        ...
        larf = lar.copy()
    if (svs == 'view') and (sns == 'seen'):
        shade_matrix_filt = shade_matrix_filt
    else:
...
p = plt.imshow(shade_matrix_filt)
plt.colorbar(p)
